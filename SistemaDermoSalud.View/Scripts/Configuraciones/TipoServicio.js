var cabeceras = ["idTipoServicio", "Codigo", "Tipo de Tratamiento", "Estado"];
var posiciones = [0, 1, 2, 3];
var listaDatos;
var matriz = [];
var url = "TipoServicio/ObtenerDatos";
enviarServidor(url, mostrarLista);
configurarBotonesModal();
var txtID = document.getElementById("txtID");
var txtCodigo = document.getElementById("txtCodigo");
var txtNombreTipoServicio = document.getElementById("txtNombreTipoServicio");
var txtDescripcion = document.getElementById("txtDescripcion");
var chkActivo = document.getElementById("chkActivo");
cfgKP(["txtNombreTipoServicio", "txtDescripcion", "chkActivo"], cfgTKP);
...
function TraerDetalle(id) {
    var url = "TipoServicio/ObtenerDatosxID/?id=" + id;
...
function actualizarListar(rpta) {
    if (rpta != "") {
        ...
        if (codigo.length == 0) {
            if (res == "OK") {
                mensaje = "Se adicionó al nuevo Tipo de Tratamiento";
                tipo = "success";
            ...
            if (res == "OK") {
                mensaje = "Se actualizó el Tipo de Tratamiento";
                tipo = "success";
                CerrarModal("modal-form");
            }
            else {
...
function configurarBotonesModal() {
    ...
    btnGrabar.onclick = function () {       
        if (validarFormulario()) {
            var url = "TipoServicio/Grabar";
            var frm = new FormData();
            frm.append("idTipoServicio", txtID.value.length == 0 ? "0" : txtID.value);
            frm.append("NombreTipoServicio", txtNombreTipoServicio.value);
            frm.append("Descripcion", txtDescripcion.value.length == 0 ? "-" : txtDescripcion.value);
...
function CargarDetalles(rpta) {
    if (rpta != "") {
        ...
        txtCodigo.value = listaDetalle[1];
        txtNombreTipoServicio.value = listaDetalle[2];
        txtDescripcion.value = listaDetalle[3];
...
function mostrarDetalle(opcion, id) {
    ...
    switch (opcion) {
        case 1:
            show_hidden_Formulario();
            lblTituloPanel.innerHTML = "Nuevo Tipo de Tratamiento";
            gbi("txtNombreTipoServicio").focus();
            break;
        case 2:
            lblTituloPanel.innerHTML = "Editar Tipo de Tratamiento";
            TraerDetalle(id);
            show_hidden_Formulario();
            gbi("txtNombreTipoServicio").focus();
            break;
...
function eliminar(id) {
    ...
    swal({
        title: "Desea Eliminar este Tipo de Tratamiento?",
    ...
    function (isConfirm) {
        if (isConfirm) {
            var url = "TipoServicio/Eliminar?idTipoServicio=" + id;
            enviarServidor(url, eliminarListar);
        } else {
            swal("Cancelado", "No se elminó el Tipo de Tratamiento", "error");
        }
    });
}
function eliminarListar(rpta) {
    if (rpta != "") {
        ...
        var mensaje = "";
        if (res == "OK") {
            mensaje = "Se eliminó el Tipo de Tratamiento";
            tipo = "success";
        }
        else {
            mensaje = data[1];
