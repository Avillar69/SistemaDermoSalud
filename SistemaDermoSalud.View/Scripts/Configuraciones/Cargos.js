var cabeceras = ["Descripcion", "Fecha","Estado"];
var listaDatos;
var matriz = [];
...
function listar(r) {
    ...
    let cols = ["cargo", "fechaCreacion", "estado"];
    loadDataTable(cols, newDatos, "idCargo", "tbDatos", cadButtonOptions(), false);    
}
...
function mostrarDetalle(opcion, id) {
    ...
    switch (opcion) {
        case 1:
            ...
            lblTituloPanel.innerHTML = "Nuevo Cargo";
            gbi("txtDescripcion").focus();
            break;
        case 2:
            let idCargo = id.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.id;
            lblTituloPanel.innerHTML = "Editar Cargo";
            TraerDetalle(idCargo);
            show_hidden_Formulario();
            gbi("txtDescripcion").focus();
            break;
    }
...
function eliminar(id) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            var url = "Cargos/Eliminar?idCargo=" + idCargo;
            enviarServidor(url, eliminarListar);
        } else {
            Swal.fire('Cancelado', 'No se eliminó el cargo', 'error');
        }
    });
}
...
function configurarBotonesModal() {
    ...
    btnGrabar.onclick = function () {
        if (validarFormulario()) {
            ...
            frm.append("idCargo", gbi("txtID").value.length == 0 ? "0" : gbi("txtID").value);
...
function actualizarListar(rpta) {
    if (rpta != "") {
        ...
        if (codigo.length == 0) {
            if (res == "OK") {
                mensaje = "Se adicionó el nuevo Cargo";
                tipo = "success";
            }
            else {
                mensaje = data[1];
...
function CargarDetalles(rpta) {
    if (rpta != "") {
        ...
        gbi("txtCodigo").value = listaDetalle[0];
        gbi("txtDescripcion").value = listaDetalle[1];
        gbi("chkActivo").checked = listaDetalle[6] == "ACTIVO" ? true : false;
    }
}
//
//exportar
...
function ExportarPDFs(orientation, titulo, cabeceras, matriz, nombre, tipo, v) {
    ...
    if (v == "e") {
        doc.save((nombre != undefined ? nombre : "cargos.pdf"));
    }
    else if (v == "i") {
        doc.autoPrint();
...
function fnExcelReport(cabeceras) {
    ...
    if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./))      // If Internet Explorer
    {
        ...
        txtArea1.focus();
        sa = txtArea1.document.execCommand("SaveAs", true, "Cargos.xls");
    }
